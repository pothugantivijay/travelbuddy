...
import os
import weather_agent  # Changed from relative import to absolute import
...
weather_agent = weather_agent.create_weather_agent(openai_api_key)
...
class WeatherEntry(BaseModel):
    ...
    temp_max: str = Field(..., description="Maximum temperature in Celsius")  # Fixed missing colon
...
def format_timestamp(ts, tz_offset):
    """
    Get Unix timestamp to human-readable time
...
def get_weather_forecast(present_location, time_zone_offset, api_key=None):
    try:  # Added missing try block
        ...
        onecall_url_v3 = f"https://api.openweathermap.org/data/3.0/onecall?lat={lat}&lon={lon}&appid={api_key}&units=metric"
        
        try:
            response = requests.get(onecall_url_v3)
            response.raise_for_status()
        except requests.RequestException as e:
            print(f"Error with One Call API 3.0: {e}. Trying API 2.5...")
            # Fall back to One Call API 2.5
            onecall_url_v2 = f"https://api.openweathermap.org/data/2.5/onecall?lat={lat}&lon={lon}&appid={api_key}&units=metric"
            response = requests.get(onecall_url_v2)
            response.raise_for_status()
        data = response.json()
        ...
        for hour in data.get('hourly', []):
            ...
            # Skip if we don't have this date in forecasts_by_date
            if date_str not in forecasts_by_date:
                continue
...
# Define MCP tools
@mcp.tool()
def get_weather(location: str, api_key: str, timezone_offset: float = 0) -> Dict[str, Any]:
    """
    ...
    weather_data = get_weather_forecast(location, timezone_offset, api_key)

    # Generate natural language weather report
    report = weather_agent.run({
        "location": location,
        "temperature": weather_data['current']['temperature'],
        "weather_condition": weather_data['current']['weather_condition'],
        "humidity": weather_data['current']['humidity'],
        "wind_speed": weather_data['current']['wind']['speed'],
        "wind_direction": weather_data['current']['wind']['direction']
    })

    return {"report": report}

@mcp.tool()
def get_current_weather(location: str, api_key: str, timezone_offset: float = 0) -> Dict[str, Any]:
    """
