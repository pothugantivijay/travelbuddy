...
class TestMCPIntegration(unittest.TestCase):
    ...
    def test_mcp_get_weather_tool(self, mock_get):
        ...
        # Call the MCP tool function with API key
        result = weather_mcp_server.get_weather(
            location=self.test_location,
            api_key=self.test_api_key,  # Explicitly passing API key
            timezone_offset=self.test_timezone_offset
        )

        # Patch the weather_agent.run method to return a fixed response
        with patch('weather_agent.create_weather_agent') as mock_create_agent:
            mock_agent = MagicMock()
            mock_agent.run.return_value = "Test weather report"
            mock_create_agent.return_value = mock_agent
            
            # Check if we're getting a report value in the result
